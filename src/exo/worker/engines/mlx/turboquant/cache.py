...
def make_turboquant_cache_from_template(
    ...
    *,
    ...
    caches: list[KVCache | TurboQuantKVCache] = []
    kv_index = 0
    for entry in template_cache:
        if isinstance(entry, KVCache):
            ...
            caches.append(entry)  # type: ignore[reportArgumentType]
    return caches
...
def make_turboquant_adaptive_cache(
    ...
    *,
    ...
    if not hasattr(model, "make_cache"):
        caches_adaptive: list[KVCache | TurboQuantKVCache] = []
        for index, _layer in enumerate(model.layers):
            if index < fp16_layers or index >= len(model.layers) - fp16_layers:
                caches_adaptive.append(KVCache())
            else:
                caches_adaptive.append(
                    TurboQuantKVCache(
                        key_bits=key_bits,
                        value_bits=value_bits,
                        seed=seed + index,
    ...
    adaptive_caches: list[KVCache | TurboQuantKVCache] = []
    for index, entry in enumerate(template_cache):
        if not isinstance(entry, KVCache):
            adaptive_caches.append(entry)  # type: ignore[reportArgumentType]
            continue

        kv_order = kv_positions.index(index)
        if kv_order < fp16_layers or kv_order >= len(kv_positions) - fp16_layers:
            adaptive_caches.append(KVCache())
        else:
            adaptive_caches.append(
                TurboQuantKVCache(
                    key_bits=key_bits,
                    value_bits=value_bits,
                    seed=seed + kv_order,
